  ...
  <div>
    ...
    <div v-if="filter.predicates && filter.predicates.length > 0" class="mb-2">
      ...
      <div class="d-flex flex-wrap gap-1 mb-2">
        <div v-for="predicate in filter.predicates" :key="predicate.id"
          ...
          <button type="button" class="btn btn-sm p-0 me-1" @click="editPredicate(predicate)"
            style="font-size: 10px; width: 16px; height: 16px;">
            <i class="bi bi-pencil"></i>
          </button>
          <button type="button" class="btn btn-sm p-0 text-danger" @click="deletePredicate(predicate.id)"
            style="font-size: 10px; width: 16px; height: 16px;">
    ...
    <div class="border-top pt-2">
      ...
      <div class="row g-1">
        ...
        <div class="col-5">
          <select v-if="newPredicate.type && availableValues.length > 0" class="form-select form-select-sm"
          ...
          <input v-else type="text" class="form-control form-control-sm" v-model="newPredicate.value"
            placeholder="مقدار" @keyup.enter="addPredicate" />
        </div>
        <div class="col-2">
          <button type="button" class="btn btn-sm btn-success w-100" v-on:click="addPredicate"
  ...
  computed: {
    availableValues: function () {
      switch (this.newPredicate.type) {
        ...
        case "uid_prefix":
          return []; // Text input, no predefined values
        default:
          return [];
      }
    },
  ...
  methods: {
    ...
    getPredicateValueLabel: function (predicate) {
      switch (predicate.type) {
        ...
        case "uid_prefix":
          return predicate.value; // Display the prefix directly
        default:
