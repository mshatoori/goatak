<template>
  <v-card class="point-details">
    <!-- Header -->
    <v-card-title class="d-flex justify-space-between align-center">
      <div class="d-flex align-center" @click="mapToUnit">
        <v-icon color="success" size="32" class="me-2">mdi-map-marker</v-icon>
    ...
    <v-card-text v-if="editing">
      <v-form ref="editForm" @submit.prevent="saveEditing">
        <v-row dense>
          ...
          <v-col cols="12">
            <v-text-field
              v-model.number="editingData.hae"
              label="ارتفاع (متر)"
              type="number"
              variant="outlined"
              density="compact"
...
const rules = {
  ...
  latitude: (value: number) =>
    (value >= -90 && value <= 90) || 'عرض جغرافیایی باید بین -90 تا 90 باشد',
  longitude: (value: number) =>
    (value >= -180 && value <= 180) || 'طول جغرافیایی باید بین -180 تا 180 باشد',
}

// Point type options
const pointTypeOptions = [
  { text: 'نقطه عمومی', value: 'b-m-p-s-m' },
  { text: 'نقطه مشاهده', value: 'b-m-p-s-p-o' },
  { text: 'نقطه کنترل', value: 'b-m-p-s-p-c' },
  { text: 'نقطه تجمع', value: 'b-m-p-s-p-a' },
  { text: 'نقطه هدف', value: 'b-m-p-s-p-t' },
  { text: 'نقطه خطر', value: 'b-m-p-s-p-h' },
  { text: 'نقطه امن', value: 'b-m-p-s-p-s' },
]
