...
func NewMessageRouter(sendToDestination func(msg *cotproto.TakMessage, dest model.SendItemDest) error, logger *slog.Logger) *MessageRouter {
...
func (r *MessageRouter) RouteMessage(msg *cot.CotMessage, config *ResendConfigDTO) error {
	...
	dest := r.CreateDestination(config.Destination)
	...
	err := r.sendToDestination(takMsg, dest)
	if err != nil {
...
func (r *MessageRouter) CreateDestination(addr *NetworkAddressDTO) model.SendItemDest {
	if addr == nil {
